...
- (instancetype)init {
    CGRect frame = CGRectMake(0, 64, DEVICE_WIDTH, 40);
...
- (void)setupItems {
    ...
    for (UIView *subView in _scrollView.subviews) {
        if ([subView isKindOfClass:[FDSlideBarItem class]]) {
            item = (FDSlideBarItem *)subView;
            item.delegate = nil;
    ...
    for (NSString *title in _itemsTitle) {
        item = [[FDSlideBarItem alloc] init];
        item.delegate = self;
...
- (void)addAnimationWithSelectedItem:(FDSlideBarItem *)item {
    ...
    CASpringAnimation *positionAnimation = [CASpringAnimation animation];
    ...
    CASpringAnimation *boundsAnimation = [CASpringAnimation animation];
